...
class MajorContributor extends StatefulWidget {
  const MajorContributor({super.key});

  @override
  State<MajorContributor> createState() => _MajorContributorState();
}

class _MajorContributorState extends State<MajorContributor> {
  ...
  Future<List<GetAllIssues>> fetchMajorContributorIssues() async {
    try {
      final issues = await fetchAllIssues();
      final graphData = await fetchGraph();

      String majorContributor = graphData.counts.commonProduct;
      List<GetAllIssues> majorContributorIssues = issues.where((issue) {
        return issue.customer == majorContributor;
      }).toList();

      return majorContributorIssues;
    } catch (e) {
              ...
              children: [
                Padding(
                  ...
                  child: FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        ...
                        return Text(
                          'Issues from ${graphData?.counts.commonProduct}',
                ...
                Padding(
                  ...
                  child: FutureBuilder<List<GetAllIssues>>(
                    future: fetchMajorContributorIssues(),
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.waiting) {
                        ...
                        return Center(
                          child: Text(
                            'No issues found for major contributor',
                            style: TextStyle(color: Colors.white),
                          ),
                        );
                      }
